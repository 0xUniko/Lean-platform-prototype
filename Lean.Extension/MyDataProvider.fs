...
type MyDataProvider() as this =
    ...
    let buildCryptoMinuteZipFromDb (market: string) (symbolLower: string) (date: DateTime) : Stream =
        ...
        let bars : QuantConnect.Data.Market.TradeBar[] = LeanSqlite.MarketData.SqliteStore.query (Some (getConnStr())) symbol res range
    ...
    interface IDataProvider with
        ...
        member _.Fetch(key: string) : Stream =
            ...
            let result =
                try
                    // Handle crypto minute from sqlite market data
