...
type GridBot() =
    ...
    override this.Initialize() =
        ...
        this.SetStartDate(2024, 1, 1)
        this.SetEndDate  (2024,12,31)

        this.SetCash(10_000m)
    ...
    override this.OnData(slice: Slice) =
        if this.IsWarmingUp then () else
        if not (slice.Bars.ContainsKey(symbol)) then () else
        ...
        if this.Transactions.GetOpenOrders(symbol).Count = 0 then
            let buyPx  = price * (1m - stepPct)
            let sellPx = price * (1m + stepPct)
